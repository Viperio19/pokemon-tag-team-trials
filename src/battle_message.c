...
static const u8 sText_StrongWildPkmnAppeared[] = _("You encountered a strong-looking {B_OPPONENT_MON1_NAME}!\p");
...
const u8 *const gBattleStringsTable[STRINGID_COUNT] =
{
    ...
    [STRINGID_NORUNNINGFROMTRAINERS]                = COMPOUND_STRING("There's no running from a Link battle!\p"),
    ...
    [STRINGID_ITEMSCANTBEUSEDNOW]                   = COMPOUND_STRING("Items can't be used now.\p"), // Not present in Gen 5+
    ...
    [STRINGID_ABILITYWEAKENEDSURROUNDINGMONSSTAT]   = COMPOUND_STRING("{B_SCR_NAME_WITH_PREFIX}'s {B_SCR_ABILITY} weakened the {B_BUFF1} of all surrounding Pokémon!"),
    ...
    [STRINGID_STATSFELL]                            = COMPOUND_STRING("{B_ATK_NAME_WITH_PREFIX}'s stats fell!"),
};
...
void BufferStringBattle(enum StringID stringID, enum BattlerId battler)
{
    ...
    case STRINGID_INTROMSG: // first battle msg
        ...
        else
        {
            ...
            else if (gBattleTypeFlags & BATTLE_TYPE_WILD_BOSS)
                stringPtr = sText_StrongWildPkmnAppeared;
            else
